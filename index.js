		...
		(err, res) => {
			...
			inquirer
				...
				.then((answers) => {
					...
					connection.query(
						query,
						[{ name: answers.whichdepartment }],
						(err, res) => {
							if (err) throw err
							console.log(
								`succefully delted ${answers.whichdepartment}`
							)
						}
					)
				})
	...
	connection.query(
		...
		(err, res) => {
			...
			inquirer
				...
				.then((answers) => {
					let q2 =
						"SELECT department.name, role.title, employee.first_name, employee.last_name, employee.role_id  "
					q2 +=
		...
		.query(`SELECT * FROM role`, (err, res) => {
			...
			inquirer
				...
				.then((answers) => {
					...
					connection.query(
						query,
						[{ title: answers.which_role }],
						(err, res) => {
							if (err) throw err
							console.log(
								`succefully deleted ${answers.which_role}`
							)
						}
					)
				})
	...
	inquirer
		...
		.then((answer) => {
			switch (answer.option) {
				...
				case "Edit current employee's role":
					updateEmpRol()
					break
					...
					.prompt([
						{
							...
							name: "option",
							message:
								"Do you want to remove another emplyee ?",
							choices: ["Yes", "No"],
						},
					])
					.then((answer) => {
